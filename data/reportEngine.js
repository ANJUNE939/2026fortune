...
/*
  Fortune Fate V8 Topic-Fixed Report Engine
  수정 목적:
  - STEP2에서 선택한 분석 항목별로 완전히 다른 답변 구조 출력
  - 내 인생 전체 흐름/2026년/재물/직업/연애/궁합/가족/개운법/리스크/종합이 서로 섞이지 않도록 분리
  - 공통 장문 카드가 모든 메뉴에 붙어 주제와 안 맞아 보이던 문제 제거
*/
(function(){
  const MENU=[
  ...
  const ELEM={
    목:{label:"성장·시작·배움·계획", trait:"새로운 것을 배우고 키우는 힘", money:"교육·기획·성장형 일·소개를 통한 확장", work:"기획, 교육, 콘텐츠, 브랜딩, 신규 프로젝트", love:"함께 성장하는 관계", risk:"계획만 많아지고 실행이 늦어질 수 있음", luck:"초록색, 산책, 식물, 새 노트, 아침 루틴"},
    화:{label:"표현·인기·홍보·열정", trait:"사람들 앞에서 드러나고 표현하는 힘", money:"홍보·영업·발표·노출을 통한 수입", work:"마케팅, 영업, 발표, 디자인, 영상, 행사", love:"표현과 확신이 있는 관계", risk:"감정이 과열되거나 말이 앞설 수 있음", luck:"핑크 포인트, 조명, 발표, 짧은 운동, 밝은 표정"},
    토:{label:"현실·책임·돈 관리·안정", trait:"현실을 붙잡고 책임지는 힘", money:"관리·장기 고객·반복 거래·안정형 수입", work:"운영, 관리, 회계, 매장, 조직 유지", love:"안정감과 책임감이 있는 관계", risk:"혼자 너무 많이 떠안거나 답답함을 참을 수 있음", luck:"베이지, 가계부, 집안 정리, 고정 루틴"},
    금:{label:"정리·기준·판단·결과", trait:"기준을 세우고 결과를 만드는 힘", money:"계약·전문성·품질·정산이 명확한 수입", work:"검수, 행정, 분석, 품질관리, 전문직", love:"애매함보다 명확한 관계", risk:"차갑게 보이거나 완벽주의가 강해질 수 있음", luck:"화이트·실버, 체크리스트, 물건 비우기, 계약서 확인"},
    수:{label:"생각·감정·회복·지혜", trait:"흐름을 읽고 깊게 생각하는 힘", money:"정보·상담·온라인·유통·연결형 수입", work:"전략, 데이터, 상담, 글쓰기, 온라인 운영", love:"편안한 대화와 깊은 감정 교류", risk:"걱정이 많아지고 결정을 미룰 수 있음", luck:"블랙, 물 마시기, 수면 회복, 기록, 조용한 공간"}
  };
  ...
  function hero(p,a,id,s,title){
    ...
    const byMenu={
      ...
      match:`궁합의 핵심은 감정보다 연락·돈·생활 규칙을 맞추는 것입니다.`,
      family:`가족운의 핵심은 혼자 책임지지 않고 역할을 나누는 것입니다.`,
      luck:`개운법의 핵심은 부족한 ${hl.low} 기운을 생활 습관으로 채우는 것입니다.`,
      risk:`리스크 관리의 핵심은 급한 결정과 감정적 지출을 줄이는 것입니다.`,
      total:`종합 흐름의 핵심은 돈·일·관계를 한 번에 넓히지 않고 순서대로 정리하는 것입니다.`
    };
    return {
      html:`<div class="result-hero"><span class="result-kicker">${esc(title)} 핵심 요약</span><div class="result-topline">1. 결론: ${esc(byMenu[id]||byMenu.life)}<br>2. 기본 성향: ${esc(st[0])}. ${esc(st[1])}<br>3. 지금 해야 할 것: ${esc(id==="money"?"지출 구조부터 확인하세요.":id==="career"?"일의 결과물을 정리하세요.":id==="love"?"상대의 행동 패턴을 보세요.":id==="match"?"두 사람의 생활 규칙을 대화하세요.":id==="family"?"내가 할 역할의 선을 정하세요.":id==="luck"?"오늘 바로 할 수 있는 작은 루틴을 시작하세요.":id==="risk"?"중요한 결정은 하루 미루세요.":"돈, 일정, 관계 중 하나를 정리하세요.")}</div><div class="result-meta">사주 8글자: ${esc(a.pillars.map(x=>x.text).join(" · "))}<br>기준 양력일: ${esc(a.solarDate||"-")}<br>일간: ${esc(a.dayStem)}(${esc(a.dayElement)}) / 구조: ${esc(a.strength.level)} / 보완 포인트: ${esc(a.yongsin.join(", "))}<br>계산 방식: ${esc(a.source)}${a.warnings&&a.warnings.length?`<br>참고: ${esc(a.warnings.join(" "))}`:""}</div></div>`,
      text:`[${title} 핵심 요약]\n1. 결론: ${byMenu[id]||byMenu.life}\n2. 기본 성향: ${st[0]}. ${st[1]}\n3. 지금 해야 할 것: ${id==="money"?"지출 구조부터 확인하세요.":id==="career"?"일의 결과물을 정리하세요.":id==="love"?"상대의 행동 패턴을 보세요.":id==="match"?"두 사람의 생활 규칙을 대화하세요.":id==="family"?"내가 할 역할의 선을 정하세요.":id==="luck"?"오늘 바로 할 수 있는 작은 루틴을 시작하세요.":id==="risk"?"중요한 결정은 하루 미루세요.":"돈, 일정, 관계 중 하나를 정리하세요."}\n사주 8글자: ${a.pillars.map(x=>x.text).join(" · ")} / 일간: ${a.dayStem}(${a.dayElement})`
    };
  }
  ...
  function report(id,p){
    ...
    const s=seedOf(p,id,a), hl=highLow(a), hi=ELEM[hl.high], lo=ELEM[hl.low], st=STEM[a.dayStem]||["자기 흐름이 있는 타입","상황을 보며 움직입니다","기준이 필요합니다"], n=p.me.name||"고객";
    ...
    if(id==="life"){
      ...
      parts.push(card("🔁","반복 패턴",`${pick(P.lifeFocus,s)}\n잘될 때는 사람과 일이 빠르게 붙지만, 기준 없이 다 받으면 피로가 커집니다.`));
      ...
      parts.push(card("📆","월별 흐름",`1~2월 정리, 3~4월 움직임, 5~6월 돈과 관계, 7~8월 과열 주의, 9~10월 성과 정리, 11~12월 다음 준비입니다.`));
      ...
      parts.push(card("📊","돈의 그릇",`${a.strength.level} 구조에서는 많이 버는 것보다 먼저 담을 그릇이 필요합니다. 가계부, 비상금, 반복 수입이 돈의 그릇입니다.`));
      ...
      parts.push(card("⚠️","일에서 조심할 점",`${pick(P.riskWay,s)}\n혼자 다 하려는 방식은 오래 버티기 어렵습니다.`));
      ...
      parts.push(card("⚠️","피해야 할 관계",`${pick(P.loveWay,s,2)}\n애매한 연락과 태도를 오래 기다리게 만드는 사람은 피로를 키울 수 있습니다.`));
      parts.push(easy(`연애는 설렘만 보면 안 됩니다. 오래 갈 사람은 연락, 돈, 생활 습관이 안정적인 사람입니다.`));
    } else if(id==="match"){
      const pa=SAJU.analyze({birth:p.partner.birth,time:p.partner.time,calendar:"양력"},"");
      let line="상대 정보가 부족합니다.", detail="상대 생년월일과 시간을 넣으면 더 구체적으로 볼 수 있습니다.";
      if(pa.ok){
        const me=a.dayElement, pe=pa.dayElement;
        if(SAJU.PRODUCES[me]===pe){line="내가 상대를 많이 챙겨주는 궁합입니다.";detail="내가 더 맞춰주는 느낌이 생길 수 있으니 상대의 고마움 표현이 중요합니다.";}
        else if(SAJU.PRODUCES[pe]===me){line="상대가 나를 도와주는 궁합입니다.";detail="상대가 힘을 주는 구조지만 받기만 하면 균형이 깨질 수 있습니다.";}
        else if(SAJU.CONTROLS[me]===pe||SAJU.CONTROLS[pe]===me){line="끌림은 강하지만 주도권 다툼이 생기기 쉬운 궁합입니다.";detail="말투, 돈, 연락 문제에서 부딪힐 수 있으니 규칙이 필요합니다.";}
        else if(me===pe){line="생각이 비슷해서 잘 통하지만 고집도 부딪힐 수 있습니다.";detail="서로 비슷해서 편하지만 둘 다 물러서지 않으면 싸움이 길어질 수 있습니다.";}
        else{line="서로 다른 점을 조율하면 좋아지는 궁합입니다.";detail="다름을 고치려 하지 말고 역할을 나누면 관계가 편해집니다.";}
      }
      parts.push(card("💞","궁합 핵심",line));
      parts.push(card("🗣️","갈등 포인트",detail));
      parts.push(card("🏠","결혼까지 볼 때",`생활비, 집안일, 양가 가족, 휴일 사용 방식은 미리 이야기해야 합니다. 연애 때 넘긴 문제가 결혼 후에는 커질 수 있습니다.`));
      parts.push(card("✅","관계 전략",`감정 확인보다 생활 규칙을 정하세요. 연락, 돈, 가족, 미래 계획을 말로만 넘기지 말고 구체적으로 맞춰야 합니다.`));
      parts.push(easy(`궁합은 좋아하는 마음보다 생활 규칙이 중요합니다. 돈, 연락, 가족 문제를 맞추면 훨씬 안정됩니다.`));
    } else if(id==="family"){
      ...
      parts.push(card("🏠","가족별 포인트",`${(p.families||[]).length?p.families.map(f=>`${f.relation} ${f.name}: 감정보다 역할과 부탁 범위를 정하는 것이 좋습니다.`).join("\n"):"가족 정보를 넣으면 가족별 포인트가 더 구체적으로 나옵니다."}`));
      ...
      parts.push(card("⚠️","피해야 할 개운법",`불안해서 비싼 물건을 사거나 검증되지 않은 말에 의존하는 것은 좋지 않습니다. 생활을 정리하는 것이 가장 안전합니다.`));
      ...
      parts.push(card("✅","최종 전략",`지출 기록, 일정 정리, 관계 기준, 수면 회복, 목표 하나 집중. 이 다섯 가지가 가장 현실적인 운 상승 방법입니다.`));
